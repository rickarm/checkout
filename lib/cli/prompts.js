...
async function askQuestion(question) {
  ...
  // Add extra newline to separate colored text from prompt
  // This helps inquirer calculate cursor position correctly on word wrap
  console.log('');
  ...
  while (!valid) {
    const { response } = await inquirer.prompt([
      {
        type: question.type === 'number' ? 'input' : 'input',
        name: 'response',
        message: 'Your answer:',
        default: '',
        prefix: ''
...
async function runCheckoutFlow(Entry) {
...
async function confirmAndSave(entry, path) {
