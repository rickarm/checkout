...
const os = require('os');
require('dotenv').config({ path: path.join(os.homedir(), '.checkout', '.env') });
...
function createServer(config) {
  ...
  app.use('/u/:user', (req, res, next) => {
    ...
    const loginUrl = `/u/${userId}/login`;
    // HTMX requests: use HX-Redirect so the browser does a full navigation
    // instead of swapping login HTML into the page content area.
    if (req.headers['hx-request'] === 'true') {
      return res.set('HX-Redirect', loginUrl).status(200).send();
    }
